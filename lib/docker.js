...
/**
 * Generate the Dockerfile according to the spec and either build
 * the deploy repo, run tests or start the application.
...
function main(options) {
    ...
    if (!config && !opts.deploy && !opts.generate) {
        console.error('Service configuration is required to run or test in docker');
        process.exit(1);
    }

    pkg = options.package;
    if (!pkg) {
        console.error('Path to package.json is required');
        process.exit(1);
    }
